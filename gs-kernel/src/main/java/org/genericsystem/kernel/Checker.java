...
public class Checker<T extends AbstractVertex<T>> {
	...
	private void checkSameEngine(T vertex) {
		...
		for (int pos = 0; pos < vertex.getComponents().size(); pos++) {
			...
			if (component == null)
				component = vertex;
			if (!component.getRoot().equals(vertex.getRoot()))
				context.discardWithException(new CrossEnginesAssignementsException("Unable to associate " + vertex + " with " + component + " because they are from differents engines"));
		}
	}
