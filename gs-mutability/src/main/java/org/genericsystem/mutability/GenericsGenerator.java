...
public class GenericsGenerator<T extends AbstractGeneric<T, ?>> {

	private final Map<T, T> map = new ConcurrentHashMap<>();
	static Logger log = LoggerFactory.getLogger(GenericsGenerator.class);

	private final T engine;

	public GenericsGenerator(T engine) {
		this.engine = engine;
	}
